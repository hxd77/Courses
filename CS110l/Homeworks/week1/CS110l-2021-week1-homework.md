...
myth机器已经具备了本次作业中我们将使用的工具，但如果你希望在自己的计算机上工作，安装它们并不困难。你需要安装 Valgrind（内存调试工具，检查内存泄漏、非法访问） 和最新版本的 LLVM（版本 10 或更新版本），该版本包含 `clang-tidy` 和清理器（静态代码分析器（在不运行程序的情况下找 Bug）。）。（如果你使用的是 Mac，默认安装不包括我们将使用的清理器，但你可以使用 `brew` 安装一个替代版本。）
