...
def _extract_tensors(dset, num=None, x_dtype=torch.float32):
    """
    从CIFAR10数据集对象中提取数据与标签，并将其转换为张量。

    输入：
    - dset：一个torchvision.datasets.CIFAR10对象
    - num：可选参数。若提供该参数，则表示要保留的样本数量
    - x_dtype：可选参数。输入图像的数据类型
    
    返回值：
    - x：数据类型为`x_dtype`、形状为(N, 3, 32, 32)的张量
    - y：数据类型为int64、形状为(N,)的张量
    """
...
def cifar10(num_train=None, num_test=None, x_dtype=torch.float32):
    """
    返回CIFAR10数据集，必要时将自动下载该数据集。
    该函数还可对数据集进行子采样。

    输入参数：
    - num_train：[可选] 从训练集中保留的样本数量。
      若未提供，则保留全部训练集。
    - num_test：[可选] 从测试集中保留的样本数量。
      若未提供，则保留全部测试集。
    - x_dtype：[可选] 输入图像的数据类型

    返回值：
    - x_train：`x_dtype` 类型的张量，形状为 (num_train, 3, 32, 32)
    - y_train：int64 类型的张量，形状为 (num_train, 3, 32, 32)
    - x_test：`x_dtype` 类型的张量，形状为 (num_test, 3, 32, 32)
    - y_test：int64 类型的张量，形状为 (num_test, 3, 32, 32)
    """
